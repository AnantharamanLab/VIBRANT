...
vibrant.add_argument('-folder', type=str, nargs=1, default=str(working_path)+"/", help="path to deposit output folder, will create if doesn't exist [default= working directory]")
...
if not os.path.exists(str(out_folder)):
	subprocess.run('mkdir ' + str(out_folder), shell=True)
...
if format == "nucl":
	...
	subprocess.run('mv ' + base + '.gff ' + str(out_folder) + 'VIBRANT_' + base +' 2>/dev/null', shell=True)
subprocess.run('mv VIBRANT*' + base + ' ' + str(out_folder) + 'VIBRANT_' + base +' 2>/dev/null', shell=True)
subprocess.run('mv VIBRANT*' + base + '.log ' + str(out_folder) + 'VIBRANT_' + base +' 2>/dev/null', shell=True)
time.sleep(0.1)
#
#
#
