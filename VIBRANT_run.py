...
vibrant.add_argument('-t', type=str, nargs=1, default='1', help='number of parallel VIBRANT runs, each occupies 1 CPU [default=1, max of 1 CPU per contig]')
...
if format == "nucl":
	try:
		...
		subprocess.run('rm VIBRANT_log_' + str(base) + '.log 2>/dev/null', shell=True)
		print("\n")
		print('No phages found. There were either no contigs at least ' + str(lim_low) + 'bp (set minimum size) or the input file (-i) format (-f) was not FASTA nucleotide. Exiting.')
		print("\n")
		exit()
...
if format == "prot":
	try:
		...
		subprocess.run('rm VIBRANT_log_' + str(base) + '.log 2>/dev/null', shell=True)
		print("\n")
		print("No phages found. VIBRANT never found any proteins. Verify input file (-i) format (-f) is FASTA protein. Exiting.")
		exit()
