...
* Scikit-Learn version warnings are no longer displayed (July 2 2020 update). To enact this update please download the new `VIBRANT_annotation.py` script from the `scripts` folder and replace with the old one. The new script only has a minor change to hide warning messages.  
______
## Known Issues:
* The pipe symbol (`|`) within the definition lines of sequence files may cause errors. If you encounter an error, check for the pipe symbol in your files and please consider removing or replacing it.  
